# Copyright (c) Mark Johnson, 30th August, 2005, modified 23rd March 2008
#
...
"""Reads, searches and displays trees from Penn Treebank format treebank files

Trees are represented in Python as nested list structures in the
following format:
...
nonterm_re matches Penn treebank nonterminal labels, and parses them
into their various parts.

empty_re matches empty elements (terminals), and parses them into
their various parts.
...
def trees(treebankdir, treebank_re):
    ...
    """
    for filename in findfiles(treebankdir, treebank_re):
...
#                 drawtree.tb_tk(tree)
#                 inp = input('{}:{} find next tree? [Y/n/p] '.format(
#                     filename, str(treeno))).strip()
...
#     nonterminal subtree of the tree. If interesting_subtree_p does not
#     return False, the tree is displayed using drawtree. If
...
def is_preterminal(subtree):
    """True if the treebank subtree is rooted in a preterminal node (i.e.,
    is an empty node or dominates a word).
...
def is_phrasal(subtree):
    """True if this treebank subtree is not a terminal or preterminal node"""
