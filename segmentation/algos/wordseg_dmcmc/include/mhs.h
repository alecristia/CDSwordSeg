...
// params, this is because constants are all stored in a single class,
...
protected:
    ...
    std::mt19937& u01;    //!< shared random number generator
        ...
        bool sanity_check() const
            {
                ...
                U mm = 0, nn = 0;
                for(const auto& val: n_m)
                {
                    assert(n > 0);   // shouldn't have any empty tables
                    assert(m > 0);
                    mm += val.second;
                    nn += val.first * val.second;
                }
...
public:
    PYAdaptor(Base& base, std::mt19937& u01, F a, F b)
    ...
    F operator() (const V& v) const
        {
            ...
            F p_old = (tit == label_tables.end()) ?
                0 : (tit->second.n - tit->second.m*a) / (n + b);
    ...
    F insert(const V& v)
        {
            ...
            F p_old = (tit == label_tables.end()) ?
                0 : (tit->second.n - tit->second.m*a);  //!< note: ignores (n - b) factor
    ...
    void erase_type_proportional()
        {
            //find max table
            F max = 0;
            for(const auto& val: label_tables)
                if (val.second.n > max) max = val.second.n;

            //compute partition func
            F tot = 0;
            for(const auto& val: label_tables)
                tot += max / val.second.n;
    ...
    F logprob() const
        {
            ...
            for(const auto& val: label_tables)
            {
                const U_U& n_m = val.second.n_m;
                for(const auto& val2: n_m)
                    logp += val2.second * (std::lgamma(val2.first - a) - std::lgamma(1 - a));
            }

            if (a > 0)
            {
                logp += m*log(a) + std::lgamma(m + b/a) - std::lgamma(b/a);
            }
            else
            {
                logp += m*log(b);
            ...
            logp -= std::lgamma(n + b) - std::lgamma(b);
            return logp;
        }
    ...
    bool sanity_check() const
        {
            ...
            for(const auto& val: label_tables)
            {
                nn += val.second.n;
                mm += val.second.m;
                sane_tables = (sane_tables && val.second.sanity_check());
            }
