...
public:
  ...
  std::wostream& print(std::wostream& os) const {
    os << "types = " << parent::ntypes() << ", tokens = " << parent::ntokens() << std::endl;
    wchar_t sep = '(';
    cforeach (typename parent::V_T, it, parent::label_tables) {
      os << sep << it->first << ' ';
      it->second.n;
      sep = ',';
    }
...
/*
//this class is needed only to keep track of the
//number of unique types, which is needed to compute
//mbdp probability.
template <typename Base>
class UnigramsT: public PYAdaptor<Base> {
private:
...
public:
  ...
  const Base& base_dist() const {return _base;}
  F& pya() {return _empty_bigram.pya();}
  F& pyb() {return _empty_bigram.pyb();}
  ...
  F insert(const V& w1, const V& w2) {
    ...
    BigramR& b = parent::insert(typename BigramsT::value_type(w1, _empty_bigram)).first->second;
  ...
  friend std::wostream& operator<< (std::wostream& os, const BigramsT& b) {
    ...
    cforeach (typename BigramsT, i, b) {
      BigramR u = (i->second); // don't know why I can't this directly in <<
       os << i->first << ": " << u << std::endl;
    }
    return os;
  }
private:
