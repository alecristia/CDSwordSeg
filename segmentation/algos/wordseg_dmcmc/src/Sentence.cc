...
std::wostream& Sentence::print(std::wostream& os) const
{
    ...
    for(U j=prev; j<size(); j++)
        os << "  ";
    os << "| " << _possible_boundaries << std::endl;
    os << "padbs: ";
...
Sentence::Sentence(U start, U end, Bs possible_boundaries,
    ...
    : Substring(start, end),
      _true_boundaries(true_boundaries),
      _constants(d)
{
...
// the sampling method used in our ACL paper.  results using this
// function reproduce those of ACL paper, but I have not actually
// checked the probabilities by hand.
U Sentence::sample_by_flips(Unigrams& lex, F temperature)
{
    U nchanged = 0;
    //cforeach(Us, iter, _possible_boundaries)
    for(const U& i: _possible_boundaries)
    {
        U i0, i1, i2, i3;
...
U Sentence::sample_by_flips(Bigrams& lex, F temperature)
{
	...
	if(debug_level >= 10000)
            std::wcout << "Sampling boundary " << i << std::endl;
...
void Sentence::sample_one_flip(Bigrams& lex, F temperature, U boundary_within_sentence)
{
    // used by DecayedMCMC to sample one boundary within a sentence
...
// Metropolis-Hastings correction.  assumes words from this sentence
// have already been erased from counts in the lexicon if necessary
// (i.e., for batch).
//
// nsentences is the number of sentences that have been seen
// (preceding this one, if online; except this one, if batch).
void Sentence::maximize(Unigrams& lex, U nsentences, F temperature, bool do_mbdp)
{
    // cache some useful constants
    int N_branch = lex.ntokens() - nsentences;
    double p_continue = pow(
        (N_branch +  _constants->aeos/2.0) / (lex.ntokens() +  _constants->aeos), 1/temperature);

    if (debug_level >=90000)
        TRACE(p_continue);

    // create chart for dynamic program
    typedef std::pair<double, U> Cell; //best prob, index of best prob
    ...
    for (j = _padded_possible.begin() + 1; j <_padded_possible.end(); j++)
    {
        for (i = _padded_possible.begin(); i < j; i++)
        {
            ...
            if (do_mbdp)
            {
                ...
                if (debug_level >=85000)
                    TRACE5(*i,*j,word_at(*i, *j),mbdp_p,prob);
            }
            else
            {
                ...
                if (debug_level >=85000)
                    TRACE5(*i,*j,word_at(*i, *j),lex(word_at(*i, *j)),prob);
            }
...
// nsentences is the number of sentences that have been seen
// (preceding this one, if online; except this one, if batch).
void Sentence::maximize(Bigrams& lex, U nsentences, F temperature)
{
    // create chart for dynamic program
    typedef std::pair<double, U> Cell; //best prob, index of best prob
    ...
    for (k = _padded_possible.begin() + 2; k <_padded_possible.end(); k++)
    {
        ...
        if (debug_level >=85000)
            TRACE3(*k,word_at(0, 1),word_at(1, *k));

        if (debug_level >=85000)
            TRACE2(lex(word_at(0, 1),word_at(1,*k)),prob);
    ...
    for (k = _padded_possible.begin() + 3; k <_padded_possible.end(); k++)
    {
        for (j = _padded_possible.begin() + 2; j < k; j++)
        {
            for (i = _padded_possible.begin() + 1; i < j; i++)
            {
                ...
                if (debug_level >=85000)
                    TRACE5(*i,*j,*k,word_at(*i, *j),word_at(*j, *k));

                if (debug_level >=85000)
                    TRACE2(lex(word_at(*i, *j),word_at(*j, *k)),prob);
    ...
    for (i = _padded_possible.begin()+1; i <j; i++)
    {
        ...
        if (debug_level >=85000)
            TRACE5(*i,*j,m,word_at(*i, *j),word_at(*j, m));

        if (debug_level >=85000)
            TRACE2(lex(word_at(*i, *j),word_at(*j, m)),prob);
...
// nsentences is the number of sentences that have been seen
// (preceding this one, if online; except this one, if batch).
void Sentence::sample_tree(Unigrams& lex, U nsentences, F temperature, bool do_mbdp)
{
    // cache some useful constants
    int N_branch = lex.ntokens() - nsentences;
    assert(N_branch >= 0);
    ...
    // create chart for dynamic program
    typedef std::pair<double, U>  Transition; // prob, index from whence prob
    ...
    while (best[k].first < 1.0) {
        ...
        for(const auto& val: best[k].second)
        {
            total += val.first;
            if (r < total)
            {
...
void Sentence::sample_tree(Bigrams& lex, U nsentences, F temperature)
{
    // create chart for dynamic program
    typedef std::pair<double, U>  Transition; // prob, index from whence prob
    typedef std::vector<Transition> Transitions;
    ...
    while (best[m][n].second[0].second > 0)
    {
        ...
        F total = 0;
        for(const auto& val: best[m][n].second)
        {
            total += val.first;
            if (r < total) {
                U previous = val.second;
                _boundaries[previous] = true;
                n = m;
                m = previous;
                break;
...
{
    scoring._sentences++;
    scoring.add_words_to_lexicon(
        get_segmented_words(), scoring._segmented_lex);

    scoring.add_words_to_lexicon(
        get_reference_words(), scoring._reference_lex);

    // calculate number of correct words, segmented words, and
    // reference words and add to totals
    const Bs& segmented = _boundaries;
    const Bs& ref = _true_boundaries;
    assert(segmented.size() == ref.size());

    if (debug_level >=50000)
        TRACE2(segmented, ref);
    ...
    if (debug_level >=60000)
        TRACE3(scoring._bs_correct, scoring._segmented_bs, scoring._reference_bs);

    if (debug_level >=60000)
        TRACE3(scoring._words_correct, scoring._segmented_words, scoring._reference_words);
}
...
Sentence::Words Sentence::get_words(const Bs& boundaries) const
{
...
//unigram insertion
void Sentence::insert(U left, U right, Unigrams& lex) const
{
    if(debug_level >= 20000)
        TRACE3(left, right, word_at(left,right));
...
//unigram removal
void Sentence::erase(U left, U right, Unigrams& lex) const
{
    if (debug_level >= 20000)
        TRACE3(left, right, word_at(left,right));
...
// bigram insertion
void Sentence::insert(U i0, U i1, U i2, Bigrams& lex) const
{
    if (debug_level >= 20000)
        TRACE5(i0, i1, i2,word_at(i0,i1), word_at(i1,i2));
...
// bigram removal
void Sentence::erase(U i0, U i1, U i2, Bigrams& lex) const
{
    if(debug_level >= 20000)
        TRACE5(i0, i1, i2,word_at(i0,i1), word_at(i1,i2));
...
F Sentence::prob_boundary(U i1, U i, U i2, const Unigrams& lex, F temperature) const
{
    ...
    if (debug_level >= 50000)
        TRACE3(p_continue, p_boundary, p_noboundary);
...
F Sentence::p_bigram(U i0, U i1, U i2, const Bigrams& lex) const
{
...
F Sentence::prob_boundary(U i0, U i1, U i, U i2, U i3, const Bigrams& lex, F temperature) const
{
...
void Sentence::surrounding_boundaries(U i, U& i0, U& i1, U& i2, U& i3) const
{
    U n = size();
    assert(i > 1);
    assert(i+1 < n);
...
F Sentence::mbdp_prob(Unigrams& lex, const Substring& word, U nsentences) const
{
    ...
    if (debug_level >=90000)
        TRACE2(total_tokens, word_tokens);
