...
ModelBase::resample_pya(Unigrams& lex) {
  ...
  lex.pya() = slice_sampler1d(pya_logP, lex.pya(), unif01, std::numeric_limits<F>::min(), 1.0, 0.0, niterations, 100*niterations);
...
Bs
ModelBase::hypersample(Unigrams& ulex, Bigrams& lex, F temp){
...
//assume beta must be > 0.  If beta must also be < 1, set flag.
// returns true if value of beta changed.
bool
ModelBase::sample_hyperparm(F& beta, bool is_prob, F temp) {
  F std_ratio = _constants->hypersampling_ratio;
  if (std_ratio <= 0) return false ;
  F old_beta = beta;
  F new_beta;
  ...
  r = pow(r,1/temp);
  //wcout << old_beta << ", " << new_beta << ", " << old_p << ", " << new_p << ", " << new_p-old_p << ", " << r << ", ";
  //if (new_beta < old_beta) wcout << "-, "; else wcout << "+, ";
  bool changed = false;
  if ((r >= 1) || (r >= randd(1))) {
    //wcout << "+";
    changed = true;
  ...
  //wcout << ";  %beta(o/n), P(o/n), diff, r, up/do, ac/re" << endl;
  return changed;
}
...
void
Model::run_eval(wostream& os, F temp, bool maximize) {
  foreach(Sentences, iter, _eval_sentences) {
    if (debug_level >= 10000) iter->print(wcerr);
    estimate_eval_sentence(*iter, temp, maximize);
  }
  assert(sanity_check());
}
...
void
BigramModel::print_statistics(wostream& os, U iter, F temp, bool header) {
  if (header) {
     os << "#Iter"
       << sep << "Temp"
       << sep << "-logP"
...
void
BigramModel::estimate_eval_sentence(Sentence& s, F temperature, bool maximize) {
  if (maximize) {
...
BatchUnigram::BatchUnigram(Data* constants): 
  ...
  cforeach(Sentences, iter, _sentences) {
    if (debug_level >= 10000) iter->print(wcerr);
    iter->insert_words(_lex);
  }
  if (debug_level >= 10000) wcerr << _lex << endl;
}

void
BatchUnigram::estimate(U iters, wostream& os, U eval_iters,
 ...
 //number of accepts in hyperparm resampling. Init to correct length.
  ...
  os << "hyperparm accept rate: ";
   if (_constants->hypersampling_ratio)
     os << accepted_anneal/nanneal << " (during annealing), " << accepted/n << " (after)" << endl;
   else
...
void
OnlineUnigram::forget_items(Sentences::iterator iter) {
  if (_forget_rate) {
    assert(!_constants->token_memory && !_constants->type_memory);
	...
	//cout << "Debug: Made it to _constants->type_memory, someplace OnlineBigram can't go";
	...
	//cout << "Debug: Made it to _constants->token_memory, someplace OnlineBigram can't go";
...
U
DecayedMCMC::find_boundary_to_sample(){
	// default: the last boundary
	U to_sample = _num_curr_pot_boundaries;

	// find which boundary to sample
	// use probability decay function to determine offset from current _npotboundaries
	// decay function:  prob_sampling_boundary(x) = p, p = (x+1)^(-_decay_rate)  [this is done to avoid division errors for 0 offset]
	// So, given a probability p, we can determine which boundary x by the following:
	//
	// (1) Given the current boundaries seen so far, use decay_offset_probs to sum all the probabilities
	...
	// decay_offset_probs[0] = 1, decay_offset_probs[1] = 0.25.  1 <= 1.2 < 1.25, so the offset chosen from
	// the current boundary should be 1.

	F rand_num = randd()*_cum_decay_prob;
	if(debug_level >= 10000) wcout << "random number chosen is " << rand_num << endl;
	...
	while(!found_bin && (index < _num_curr_pot_boundaries)){
		// compare rand_num with decay_offset_probs[index] and decay_offset_probs[index+1]
		if(debug_level >= 10000) wcout << "index is " << index << ", comparing " << rand_num << " with " << curr_cum_sum
				<< " and " << (_decay_offset_probs[index] + curr_cum_sum) << endl;
		if((curr_cum_sum <= rand_num) && (rand_num < (curr_cum_sum + _decay_offset_probs[index]))){
			// found bin
			found_bin = true;
...
void
DecayedMCMC::find_sent_to_sample(U boundary_to_sample, Sentence &s, Sentences& sentences_seen){
	Sentences::iterator si = sentences_seen.end();
	// get to last element
	si--;
	if(debug_level >= 10000) wcout << "Current sentence looking for boundary " << boundary_to_sample << " in is " << *si << endl;
	s = *si;
	...
	while(!sent_found){
		...
		foreach(Us, count, possible_boundaries){
			this_sent_boundaries++;
		}
		if(debug_level >= 10000) wcout << "Seeing if boundary " << boundary_to_sample << " is greater than "
									<< num_boundaries << " - " << this_sent_boundaries << endl;
		if(boundary_to_sample > (num_boundaries - this_sent_boundaries)){
			sent_found = true;
			_sentence_sampled = si;
			if(debug_level >= 10000) wcout << "Found sentence containing boundary "<< boundary_to_sample << ": " << *si << endl;
			_boundary_within_sentence = this_sent_boundaries - (num_boundaries - boundary_to_sample);
			if(debug_level >= 10000) wcout << "Will be sampling boundary " << _boundary_within_sentence << " inside this sentence " << endl;
		}else{
			// haven't found it yet - go to next sentence and set up new comparisons
			if(debug_level >= 10000) wcout << "Haven't found boundary " << boundary_to_sample
			...
			if(si != sentences_seen.begin()){
			  ...
			  if(debug_level >= 1000) wcout << "***Couldn't find boundary " << boundary_to_sample << ", so returning first utterance " << endl;
				sent_found = true;
				_sentence_sampled = si;
			}
		} 
...
void
DecayedMCMC::replace_sampled_sentence(Sentence s, Sentences &sentences_seen){

	Sentences::iterator si = sentences_seen.end();
	while((si != _sentence_sampled) && (si != sentences_seen.begin())){
		si--;
	}
	if(debug_level >= 10000) wcout << "Replacing this sentence in sentences_seen: " << *si << endl << " with this one: " << s << endl;
	*si = s;
}

void
OnlineUnigramDecayedMCMC::estimate_sentence(Sentence& s, F temperature) {
//cerr << "Debug: Made it into OnlineUnigramDecayedMCMC\n";
	// update current total potential boundaries
	Us possible_boundaries = s.get_possible_boundaries();
//cerr << "Debug: Found possible_boundaries\n";
	U num_boundaries = 0;
	foreach(Us, count, possible_boundaries){
		num_boundaries++;
	}
//cerr << "Debug: Counted num_boundaries\n";
	...
	for(U num_samples = 0; num_samples < _samples_per_utt; num_samples++){
		...
		// locate utterance containing this boundary, which includes figuring out which boundary position j in the utterance
		...
		if(debug_level >= 10000){
			...
			foreach(Sentences, _s, _sentences_seen){
				wcout << *_s << endl;
			}
		}
	}
	if(debug_level >= 10000) wcout << "_lex is now: " << endl << _lex << endl;
//cerr << "Debug: Made it through OnlineUnigramDecayedMCMC\n";
}

void
OnlineUnigramViterbi::estimate_sentence(Sentence& s, F temperature) {
  s.maximize(_lex, _nsentences_seen, temperature,_constants->do_mbdp);
  s.insert_words(_lex);
}

void
OnlineUnigramTreeSampler::estimate_sentence(Sentence& s, F temperature) {
...
void
BatchBigram::estimate(U iters, wostream& os, U eval_iters,
						F temp, bool maximize, bool is_decayed) {
	if(debug_level >= 10000) wcout << "Inside BatchBigram estimate " << endl;
  if (_constants->trace_every > 0) {
    print_statistics(os, 0, 0, true);
  }
//number of accepts in hyperparm resampling. Init to correct length.
  ...
  U n = 0;
   ...
   os << "hyperparm accept rate: " << accepted_anneal/nanneal << " (during annealing), " << accepted/n << " (after)" << endl;
}

void
BatchBigramViterbi::estimate_sentence(Sentence& s, F temperature) {
  s.erase_words(_lex);
  s.maximize(_lex, _constants->nsentences()-1, temperature);
  s.insert_words(_lex);
}
...
void
BatchBigramTreeSampler::estimate_sentence(Sentence& s, F temperature) {
  s.erase_words(_lex);
  s.sample_tree(_lex, _constants->nsentences()-1, temperature);
  s.insert_words(_lex);
}
...
void
BatchBigramFlipSampler::estimate_sentence(Sentence& s, F temperature) {
  s.sample_by_flips(_lex, temperature);
}

OnlineBigramDecayedMCMC::OnlineBigramDecayedMCMC(Data* constants, F forget_rate, F decay_rate, U samples_per_utt):
...
void
OnlineBigram::estimate(U iters, wostream& os, U eval_iters,
						F temp, bool maximize, bool is_decayed) {
	if(debug_level >= 10000) wcout << "Inside OnlineBigram estimate " << endl;
  if (_constants->trace_every > 0) {
    print_statistics(os, 0, 0, true);
  }
  _nsentences_seen = 0;
...
void
OnlineBigram::forget_items(Sentences::iterator iter) {
  if (_forget_rate) {
    if (_sentences.begin()+_forget_rate <= iter) {
      if (debug_level >= 9000) TRACE(*(iter-_forget_rate));
      (iter-_forget_rate)->erase_words(_lex);
...
void
OnlineBigramViterbi::estimate_sentence(Sentence& s, F temperature) {
  s.maximize(_lex, _nsentences_seen, temperature);
  s.insert_words(_lex);
}

void
OnlineBigramTreeSampler::estimate_sentence(Sentence& s, F temperature) {
  s.sample_tree(_lex, _nsentences_seen, temperature);
  s.insert_words(_lex);
}

void
OnlineBigramDecayedMCMC::estimate_sentence(Sentence& s, F temperature) {
	// update current total potential boundaries
	Us possible_boundaries = s.get_possible_boundaries();
	U num_boundaries = 0;
	foreach(Us, count, possible_boundaries){
