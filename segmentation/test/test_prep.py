...
import segmentation.wordseg_prep as prep
import pytest
...
def test_bad_utterances(utt):
    with pytest.raises(ValueError):
        prep.check_utterance(utt, separator=Separator())
...
def test_good_utterances(utt):
    assert prep.check_utterance(utt, separator=Separator())
...
def test_prepare_text():
    ...
    def f(u):
        return list(prep.prepare_text(p['raw'], separator=Separator(), unit=u))
