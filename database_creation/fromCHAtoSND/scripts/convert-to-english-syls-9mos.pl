...
# converts unicode file back to english with syllabification breaks.
...
binmode(IN, ":utf8");
open(OUT, ">$outfolder$corpusname-text-klatt-syls.txt") || die "Couldn't open $outfolder$corpusname-text-klatt-syls.txt\n";
binmode(OUT, ":utf8");
binmode(STDOUT, ":utf8");
