...
then
echo training $ALGO
head -200 $res/tags.txt | sed 's/;esyll//g' > $res/clean_train.txt
...
then
GRAMMARFILE=$../algos/AG/do_AG_english.sh # to add a variable indicating the unit representation : syllable or phoneme
...
# actual algo running
$GRAMMARFILE $res $ALGO $DEBUG $UNIT|| exit 1
