...
#if !defined(tgamma)
/* GCC 4.4.0 on Linux complains about incompatible implicit declarations for these,
   so give the right declarations for them.
   And this must be done before the macro is defined, which on Linux is not
   done in <math.h> for some reason. */
