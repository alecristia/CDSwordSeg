...
// Contains classes written by Mark Johnson and Sharon Goldwater
...
public:
  ...
  UniformMultinomial(F dimensions):
    _dimensions(dimensions), _prob(1.0/_dimensions), _nitems(0) {
    assert(_dimensions >0);}
  U dimensions() const {return _dimensions;}
  //! operator() returns the probability of an item
  F operator()(const X& v) const {return _prob;}
  void insert(const X& v) {++_nitems;}
  void erase(const X& v) { --_nitems;}
  bool empty() const { return _nitems == 0;}
  void clear() {_nitems = 0;}
  F logprob() const {return _nitems * log(_prob);}
private:
...
//! CharSeq{} implements a geometric distribution over character sequences
// using a unigram character model, with uniform distr. on chars.
//
...
struct CharSeq {
  ...
  typedef F  result_type;
  CharSeq(F p_nl, U nc) : p_nl(p_nl), nc(nc), nchars(), nstrings() { 
    assert(p_nl > 0);
    assert(p_nl <= 1);
    assert(nc > 0);
  }
  F p_stop() const {return p_nl;}
  F& p_stop() {return p_nl;}
  U nchartypes() const {return nc;}
  U get_nchars() {return nchars;}
  U get_nstrings() {return nstrings;}
  //! operator() returns the probability of a substring.  We special-case the situation
  //! where the initial substring is the end marker (assumed to be the character in position 0).
  F operator()(const Xs& v) const { return v == Xs(0,1) ? p_nl : pow((1.0-p_nl)/nc, v.size())*p_nl; }
  ...
  bool empty() const { return nchars == 0 && nstrings == 0; }
  void clear() { nchars = nstrings = 0; }
...
struct CharSeq0 {
  ...
  typedef Xs argument_type;
  typedef F  result_type;
  CharSeq0(F p_nl, U nc) : p_nl(p_nl), nc(nc), nchars(), nstrings() { 
    assert(p_nl > 0);
    assert(p_nl <= 1);
    assert(nc > 0);
  }
  ...
  F operator()(const Xs& v) const { 
    assert(v.size() > 0); assert(*v.begin() != '\n'); 
...
public:
  ...
  F p_stop() const {return p_nl;}// dummy, for compatibility w/ CharSeq
  F& p_stop() {return p_nl;}// dummy, for compatibility w/ CharSeq
  U nchartypes() const {return nc;}
  U get_nstrings() {return nstrings;}
  //! operator() returns the probability of a substring.  
...
// Learns a bigram distriburtion over chars to compute the string prob.
template <typename Xs>
class BigramChars {
private:
  typedef UniformMultinomial<wchar_t> Char;
