	...
	foreach $model (@order_models){
		foreach $error (@order_errors){
			foreach $vers (@order_vers){
				$file = "okayerrors/$lang/$model\:$vers\_$error\_context\.txt";
...
open(OUT, ">okayerrors/summary_context.txt")or die("couldn't open summary.txt\n");
foreach $lang (@order){
	foreach $error (@order_errors){
		foreach $model (@order_models){
...
open(OUT, ">okayerrors/summary_context_condensed.txt")or die("couldn't open summary.txt\n");
foreach $lang (@order){
	foreach $error (@order_errors){
