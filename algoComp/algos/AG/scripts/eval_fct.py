...
def words_stringpos(ws):
    ...
    for w in ws:
        ...
        for categorie in categories:
            if tag == categorie:
                dict_stringpos[categorie].add((left, right))
        left = right
    return stringpos, dict_stringpos


def read_data(lines, tree_flag, score_cat_rex=None, ignore_terminal_rex=None,
              word_split_rex=None, debug_level=0):
    ...
    if tree_flag:
        ...
        words = [[w for w in word_split_rex.split(line)
                  if w != '' and not ignore_terminal_rex.match(w)]
                 for line in lines if line.count("lexentry") == 0]
    ...
    aux = [words_stringpos(ws) for ws in words]
    stringpos = [set_pos for set_pos, _ in aux]
    dict_stringpos =  {}
    for categorie in categories:
        dict_stringpos[categorie] = [dict_pos[categorie] for _, dict_pos in aux]
    return (segments, stringpos, dict_stringpos)
...
class PrecRec:
    ...
    # def __str__(self):
    # return ("%.4g\t%.4g\t%.4g\t%.4g" % (self.fscore(), self.precision(),
    # self.recall(), self.exact_match()))

    def __str__(self):
        return ("%.4g\t%.4g\t%.4g" % (self.fscore(), self.precision(),
                                      self.recall()))
...
def evaluate(options, trainwords, trainstringpos, goldwords, goldstringpos):
    ...
    if options.debug >= 1000:
        for (tw, tsps, gw, gsps) in zip(trainwords, trainstringpos, goldwords,
                                        goldstringpos):
    ...
    #             sys.stderr.write(
    #                 "# first difference at goldwords[%s] = "
...
if __name__ == '__main__':
    ...
    parser.add_option(
        "-n", "--name", dest="levelname",
        help="name of the adaptor being evaluated")
    ...
    parser.add_option(
        "-c", "--score-cat-re", dest="score_cat_re", default=r"^Word$",
        help="score categories in tree input that match this regex")
    parser.add_option(
        "-i", "--ignore-terminal-re", dest="ignore_terminal_re",
        default=r"^[$]{3}$", help="ignore terminals that match this regex")
    parser.add_option(
        "-w", "--word-split-re", dest="word_split_re", default=r"[ \t]+",
    ...
    parser.add_option(
        "-d", "--debug", dest="debug",
        help="print debugging information", default=0, type="int")
    ...
    (goldwords, goldstringpos, golddictstringpos) = \
        read_data([line.strip() for line in goldf],
    ...
    # print PrecRecHeader
    sys.stdout.write(
        "token_f-score\ttoken_precision\ttoken_recall\tboundary_f-score"
    ...
    for trainline in trainf:
        ...
        (trainwords, trainstringpos, traindictstringpos) = \
            read_data(trainlines,
        ...
        for categorie in categories:
            evaluate(options, trainwords, traindictstringpos[categorie],
                     goldwords, golddictstringpos[categorie])
        trainlines = []
