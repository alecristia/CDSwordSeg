...
def list_freq_token_per_algo(algo,sub,path_res,size,freq_file="/freq-top.txt"):
    algo_list=[]
    if algo!="ngrams": 
...
def compare_token_btw_algo(size,path_res, dic_corpus, sub=["sub0","sub1","sub2","sub3","sub4","sub5","sub6","sub7","sub8","sub9"],
                          algos=['dibs','ngrams','tps','puddle','dmcmc','AGu'],algo_ref="dibs",freq_file="/freq-top.txt"): 
    res=[]
    for i in range(len(sub)):
        dic_inter={}
        ref=list_freq_token_per_algo(algo_ref,sub[i],path_res,size,freq_file)[0]
        for j in range(len(algos)):
            if algos[j]!=algo_ref:
                b=list_freq_token_per_algo(algos[j],sub[i],path_res, size,freq_file)[0]
...
def intersection_all_algo(size,path_res, dic_corpus, sub=["sub0","sub1","sub2","sub3","sub4","sub5","sub6","sub7","sub8","sub9"],
                          algos=['dibs','ngrams','tps','puddle','dmcmc','AGu'],algo_ref="dibs",freq_file="/freq-top.txt"):
    ...
    n=len(algos)-2
    for ss in sub:
        ref_ortho=compare_token_btw_algo(size,path_res, dic_corpus,[ss],algos,algo_ref,freq_file)[0].values()[n]["ortho"]
        ref_wrong_seg=compare_token_btw_algo(size,path_res, dic_corpus,[ss],algos,algo_ref,freq_file)[0].values()[n]["wrong_segmentation"]
        for algo in algos:
            if not algo==algo_ref:
                a_ortho[algo]=compare_token_btw_algo(size,path_res, dic_corpus,[ss],algos,algo_ref,freq_file)[0][algo]["ortho"]
                a_wrong_seg[algo]=compare_token_btw_algo(size,path_res, dic_corpus,[ss],algos,algo_ref,freq_file)[0][algo]["wrong_segmentation"]
...
def compare_token_btw_sub(size,path_res,dic_corpus,sub=["sub0","sub1","sub2","sub3","sub4","sub5","sub6","sub7","sub8","sub9"],sub_ref="sub0",
    ...
    for j in range(len(algos)):
        ...
        ref=list_freq_token_per_algo(algos[j],sub_ref,path_res,size,freq_file)[0]
        for i in range(len(sub)):
...
def compare_token_all_sub(size,path_res,dic_corpus,sub=["sub0","sub1","sub2","sub3","sub4","sub5","sub6","sub7","sub8","sub9"],algos=['dibs','ngrams','tps','puddle','dmcmc','AGu'],freq_file="/freq-top.txt"):
    n=len(sub)-1
    res={}
    for i in range(len(algos)):
        comp_all_sub_per_algo=list_freq_token_per_algo(algos[i],sub[n],path_res, size,freq_file)[0]# list of freq words for algo[i] computed in  sub[n]
        for j in range(n):
            a=list_freq_token_per_algo(algos[i],sub[j],path_res, size,freq_file)[0]
...
########################    
def differentiate_token_btwn_algo(size,path_res,dic_corpus,sub=["sub0","sub1","sub2","sub3","sub4","sub5","sub6","sub7","sub8","sub9"],algo_ref="dibs",
                          algos=['dibs','ngrams','tps','puddle','dmcmc','AGu'],freq_file="/freq-top.txt"):
    res=[]
    for i in range(len(sub)):
        dic_inter={}
...
def signature_algo(size,path_res,dic_corpus,sub=["sub0","sub1","sub2","sub3","sub4","sub5","sub6","sub7","sub8","sub9"],algo_ref="dibs",
                          algos=['dibs','ngrams','tps','puddle','dmcmc','AGu'],freq_file="/freq-top.txt"):
    res=[]
    n=len(algos)
    for i in range(len(sub)):
...
def in_common_two_algo(size, path_res,dic_corpus, sub, algos, algo1, algo2, freq_file):
    dic_inter={}
    ref1=list_freq_token_per_algo(algo1,sub,path_res,size,freq_file)[0]
    ref2=list_freq_token_per_algo(algo2,sub,path_res,size,freq_file)[0]
...
def intersection_exclusive_in_2_algo(size, path_res, dic_corpus, sub, algos, freq_file="/freq-top.txt"):
    '''for one sub !!!! '''
    tuple_res=()
    res=[]
    ...
    for algo1 in z:  
        ...
        for algo2 in z:
            #z.remove(algo2)
            if algo2!=algo1:
                res.append(in_common_two_algo(size, path_res,dic_corpus, sub, z, algo1, algo2, freq_file))
...
def average_inter_per_sub(size, path_res, dic_corpus, sub, algos, freq_file="/freq-top.txt"):
    res=[]
    for ss in sub: 
        res.append(intersection_exclusive_in_2_algo(size, path_res, dic_corpus, ss, algos, freq_file="/freq-top.txt"))
  
def count_type_segmented_per_algo_per_sub(algos,sub,path_res,size, freq_file="/freq-top.txt"):
    res=[]
    file=open("NumberTypesPerAlgoPerSub.txt","w")
    for i in sub: 
        file.write("\n"+i+"\n")
        for j in algos: 
            count=list_freq_token_per_algo(j,i,path_res,size,freq_file)[1]
            file.write(j+" " +str(count)+"\n")
            res.append(count)
    file.close()
    return(res)  
    
def count_type_well_segmented_per_algo_per_sub(dic,algos,sub,path_res,size, freq_file="/freq-top.txt"):
    res=[]
    file=open("NumberTypesPerAlgoPerSub.txt","w")
    for i in sub: 
        file.write("\n"+i+"\n")
        for j in algos: 
            list_type=list_freq_token_per_algo(j,i,path_res,size,freq_file)[0]
...
size=400 # nb of token selected in freq-list.txt
...
#### freq file
freq_ngrams_0=list_freq_token_per_algo("ngrams","sub0",path_res,size,"/freq-top.txt")[0]
freq_tps_0=list_freq_token_per_algo("tps","sub0",path_res,size,"/freq-top.txt")[0]
freq_dibs_0=list_freq_token_per_algo("dibs","sub0",path_res,size,"/freq-top.txt")[0]
freq_puddle_0=list_freq_token_per_algo("puddle","sub0",path_res,size,"/freq-top.txt")[0]
...
def mean_token_segmented_per_sub(algos, sub,path_res, dic, freq_file):
    ...
    for algo in algos: 
        for ss in sub: 
            freq[ss]=list_freq_token_per_algo(algo,ss,path_res,size,freq_file)[0]
...
countType=count_type_segmented_per_algo_per_sub(ALGOS,SUBS,path_res,size, freq_file="/freq-top.txt")    
countTypeSplit=count_type_well_segmented_per_algo_per_sub(dic_corpus,ALGOS,SUBS,path_res,size, freq_file="/freq-top.txt")
                        
intersection_btw_algo=compare_token_btw_algo(size,path_res,dic_corpus,SUBS,ALGOS,"dibs","/freq-top.txt")
     
intersection_btw_sub=compare_token_btw_sub(size,path_res,dic_corpus,SUBS,sub_ref="sub0",algos=ALGOS,freq_file="/freq-top.txt")
    
intersection_all_sub=compare_token_all_sub(size,path_res,dic_corpus,sub=SUBS,algos=ALGOS,freq_file="/freq-top.txt" )

intersection_all_algo=intersection_all_algo(size,path_res, dic_corpus, sub=SUBS,algos=ALGOS,algo_ref="dibs",freq_file="/freq-top.txt")

#for one sub !!!!
list_inter_exclu=intersection_exclusive_in_2_algo(size, path_res,dic_corpus,"sub0",ALGOS, freq_file="/freq-top.txt") 


## tokens segmented by all algos in all 
ALGOS=['dibs','tps','puddle']
 dibs_signature=signature_algo(size,path_res,dic_corpus,sub=SUBS,algo_ref="dibs",algos=ALGOS,freq_file="/freq-top.txt")
 tps_signature=signature_algo(size,path_res,dic_corpus,sub=SUBS,algo_ref="tps",algos=ALGOS,freq_file="/freq-top.txt")
 puddle_signature=signature_algo(size,path_res,dic_corpus,sub=SUBS,algo_ref="puddle",algos=ALGOS,freq_file="/freq-top.txt")
  
ALGOS=['ngrams', 'dibs','tps','puddle']
ngrams_signature=signature_algo(size,path_res,dic_corpus,sub=SUBS,algo_ref="ngrams",algos=ALGOS,freq_file="/freq-top.txt")
...
intersection_btw_algo_ngrams=compare_token_btw_algo(size,path_res,dic_corpus,SUBS,ALGOS,"ngrams","/freq-top.txt",)
intersection_btw_algo_puddle=compare_token_btw_algo(size,path_res,dic_corpus,SUBS,ALGOS,"puddle","/freq-top.txt",)
ngrams_p_o=[]
ngrams_p_ws=[]
for i in range(len(SUBS)): 
...
puddle=(19,276)
dibs=(16,277)
nb_dibs_puddle=(27,34)
nb_dibs_ngrams=(63,25)
nb_dibs_tps=(26,26)
nb_ngrams_tps=(49,52)
nb_ngrams_puddle=(76,25)
nb_tps_puddle=(34,24)
...
def variance_len_signature_per_sub(signature):
    n=len(signature)
    len_o=[]
    len_ws=[]
    res={}
    for i in range(n):
        ...
        len_ws.append(len(signature[i].values()[1]))
    print(len_o)
    c_o=sum(len_o)/len(len_o)
    ss_o = sum((x-c_o)**2 for x in len_o)
    c_ws=sum(len_ws)/len(len_ws)
    ss_ws = sum((x-c_ws)**2 for x in len_ws)
    res["ortho"]=ss_o
    res["ws"]=ss_ws
    return res
...
mean_ngrams_signature=average_signature_per_sub(tps_signature)
