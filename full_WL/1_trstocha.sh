...
# exit
    ...
    sed -r 's/I\|.\|/toglue/' < $BASE.step1 |
        awk '{if($NF~"toglue") \
             {mem=mem $0 " "} \
             else{print mem $0; mem="" }} \
             END{print mem}' |
        sed "s/toglue//g" > $BASE.glued
...
do
    ...
    do
        file2=$CHAFOLDER/WL_${DS}_LS/`basename $file | cut -d_ -f1`.cha
        mv $file $file2
    done
    ...
    do
        file2=$CHAFOLDER/WL_${DS}_HS/`basename $file | cut -d_ -f1`.cha
        mv $file $file2
    done
done

# clean up
rm -f $CHAFOLDER/*.step1 $CHAFOLDER/*.glued $CHAFOLDER/*.txt
